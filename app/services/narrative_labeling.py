...
from app.services.llm import BaseLLMClient, LLMError, NarrativeLabel
...
class NarrativeLabelingService:
    ...
    def label_cluster(self, cluster: GroupedClaimCluster) -> NarrativeLabel:
        ...
        representative_claims = [
            claim.normalized_claim_text or claim.claim_text
            for claim in cluster.claims[:5]
        ]
        prompt = self._build_prompt(cluster, representative_claims)
        system_prompt = (
            "Ты помогаешь размечать narrative results. Верни JSON-объект вида "
            "{\"title\": ..., \"formulation\": ..., \"explanation\": ...}. "
            "Title: 3-8 слов. Formulation: 1-2 предложения. Explanation: коротко, почему это сильный нарратив. "
            "Не придумывай смысл вне claims и не делай выводы шире данных."
        )
        try:
    ...
    def _build_prompt(cluster: GroupedClaimCluster, representative_claims: list[str]) -> str:
        lines = [
            f"Тип нарратива: {cluster.claim_type}",
            f"Cluster summary: {cluster.representative_text}",
        ...
        lines.append("")
        lines.append("Верни JSON для title, formulation и explanation.")
        return "\n".join(lines)

    @staticmethod
    def _fallback_label(cluster: GroupedClaimCluster) -> NarrativeLabel:
        formulation = cluster.representative_text.strip()
        explanation = (
            f"Нарратив типа {cluster.claim_type} основан на {len(cluster.claims)} claims "
            f"из {len(cluster.articles)} статей."
        )
        return NarrativeLabel(
            title=" ".join(formulation.split()[:8]).strip() or cluster.claim_type,
