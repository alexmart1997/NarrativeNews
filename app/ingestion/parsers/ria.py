...
class RiaParser(BaseArticleParser):
    TITLE_PATTERNS = [
        re.compile(r'<h1[^>]+class="[^"]*article__title[^"]*"[^>]*>(?P<content>.*?)</h1>', re.IGNORECASE | re.DOTALL),
        re.compile(r'<h1[^>]*>(?P<content>.*?)</h1>', re.IGNORECASE | re.DOTALL),
    ]
    SUBTITLE_PATTERNS = [
        re.compile(r'<div[^>]+class="[^"]*article__announce-text[^"]*"[^>]*>(?P<content>.*?)</div>', re.IGNORECASE | re.DOTALL),
        re.compile(r'<div[^>]+class="[^"]*article__second-title[^"]*"[^>]*>(?P<content>.*?)</div>', re.IGNORECASE | re.DOTALL),
    ]
    AUTHOR_PATTERNS = [
        re.compile(r'<a[^>]+class="[^"]*article__author[^"]*"[^>]*>(?P<content>.*?)</a>', re.IGNORECASE | re.DOTALL),
        re.compile(r'<span[^>]+class="[^"]*author__name[^"]*"[^>]*>(?P<content>.*?)</span>', re.IGNORECASE | re.DOTALL),
    ]
    CATEGORY_PATTERNS = [
        re.compile(r'<a[^>]+class="[^"]*article__tag[^"]*"[^>]*>(?P<content>.*?)</a>', re.IGNORECASE | re.DOTALL),
        re.compile(r'<a[^>]+class="[^"]*breadcrumbs__link[^"]*"[^>]*>(?P<content>.*?)</a>', re.IGNORECASE | re.DOTALL),
    ]
    BODY_CONTAINER_PATTERNS = [
        re.compile(r'<div[^>]+class="[^"]*article__body[^"]*"[^>]*>(?P<content>.*?)</div>\s*</div>', re.IGNORECASE | re.DOTALL),
        re.compile(r'<div[^>]+class="[^"]*article__text__overview[^"]*"[^>]*>(?P<content>.*?)</div>', re.IGNORECASE | re.DOTALL),
    ]
    BODY_BLOCK_PATTERNS = [
        re.compile(r'<div[^>]+class="[^"]*article__text[^"]*"[^>]*>(?P<content>.*?)</div>', re.IGNORECASE | re.DOTALL),
        re.compile(r'<p[^>]*>(?P<content>.*?)</p>', re.IGNORECASE | re.DOTALL),
    ]
    REMOVE_PATTERNS = [
        re.compile(r'<[^>]+class="[^"]*(?:article__share|footer|article__tags|article__story|article__service|article__info-block|article__banner)[^"]*"[^>]*>.*?</[^>]+>', re.IGNORECASE | re.DOTALL),
        re.compile(r'<[^>]+class="[^"]*(?:article__picture-description|article__picture-caption|article__photo-credit|article__media-bank)[^"]*"[^>]*>.*?</[^>]+>', re.IGNORECASE | re.DOTALL),
        re.compile(r'<div[^>]*>\s*(?:Краткий пересказ от РИА ИИ|Читать ria\.ru в|Медиабанк|Теги|Еще по теме)\s*</div>', re.IGNORECASE | re.DOTALL),
    ]
    ...
    def _extract_category(self, html: str) -> str | None:
        return (
            extract_meta_content(html, ["article:section"])
            or extract_first_match(html, self.CATEGORY_PATTERNS)
        )
    ...
    def _is_valid_paragraph(text: str) -> bool:
        ...
        return len(text) >= 20 and all(fragment not in text for fragment in invalid_fragments)
